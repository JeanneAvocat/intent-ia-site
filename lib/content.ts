...
/**
 * Couche de contenu MDX. Une page = un fichier `.mdx` dans `content/`, versionné
 * dans Git : on rédige, on pousse, Clever Cloud déploie. Les fichiers sont lus au
 * build (SSG) ; le frontmatter pilote les métadonnées d'article, le corps MDX
 * porte la prose des articles.
 *
 ...
 *
 * Note de provenance : TOUS les articles de `content/blog/` livrés avec ce
 * dépôt initial portent `provisional: true`. Ce sont des exemples de démonstration
 * écrits dans la voix de la marque, SANS statistique, témoignage ou citation
 * attribuée à une vraie personne ou entreprise. Ils doivent être relus et
 * validés par Hugo (passer `provisional: false`) avant toute publication réelle.
 */
